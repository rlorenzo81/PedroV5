...
@TeleOp(name="RED v5", group="Robot")
@Disabled

public class RCREDv5Works extends OpMode {
    ...
    // (kept constants untouched; compensated helper replaced below)
    ...
    public void loop() {
        ...
        if (b2Now) {
            ...
            if (lb2Pressed) {
                if (intakeMode == IntakeMode.INTAKE_SLOW) intakeMode = IntakeMode.OFF;
                else intakeMode = IntakeMode.INTAKE_SLOW;
            }

            if (rb2Pressed) {
                if (intakeMode == IntakeMode.SHOOTER_FULL) intakeMode = IntakeMode.OFF;
                else intakeMode = IntakeMode.SHOOTER_FULL;
            }
        }

        // Apply motor powers based on mode
        if (intakeMode == IntakeMode.INTAKE_SLOW) {
            fi.setPower(1.0);
            bi.setPower(-0.65);
        ...
        // =========================
        // SPINUP KICK -> PIDF (REPLACES COMPENSATED SHOOTER HELPERS)
        // =========================
        boolean shooterOn = (shooterSetpoint > 0.0);

        if (shooterOn) {
            // Your requested numbers
            spinup.kickMs = 1800;        // longer kick for testing
            spinup.kickPower = 1.0;
            spinup.earlyTakeoverPct = 0.98;

            // Start spinup ONLY when first turning on OR setpoint changes
            if (!shooterWasOnDiag || shooterSetpoint != lastSpinupSetpointDiag) {
                spinup.start(shooterSetpoint); // uses your setpoint as target ticks/sec
            ...
            shooterTargetVelCmd = shooterSetpoint; // keep cmd field populated (now equals target)
        ...
        // =========================
        // SPINUP DIAGNOSTICS TELEMETRY (ADDED)
        // =========================
        ...
        // Added diagnostics lines
        telemetry.addData("Shooter Phase", spinup.getPhase());
        telemetry.addData("Shooter Elapsed(ms)", spinup.getElapsedMs());
    ...
    // ===================== SPINUP / BATTERY HELPERS (REPLACEMENT) =====================
    ...
    // =========================================================
    // INNER CLASS: Kick -> PIDF Spinup Controller + Phase
    // =========================================================
