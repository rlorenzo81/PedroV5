...
@Autonomous(name = "Red Back V6 ", group = "Examples")
public class RedBackV6FastCycleMoveBackGate extends OpMode {
    ...
    double target = 1100.0; // change if needed
    ...
    private static final double SHOOT_FEED_SEC = 1.0; //was 1.5

    // ================= SHOOTER SPIN-UP DELAY =================
    // ONLY used for FIRST shot window now
    private static final double SHOOTER_SPINUP_SEC = 3.0; //was 3.0
    ...
    // NEW: gate -> intake post, then intake post -> shoot
    ...
    public void buildPaths() {
        ...
        // OLD (kept but unused in new flow)
        ...
        gate1ToIntakePost = follower.pathBuilder()
                .addPath(new BezierLine(driveToGate1, intakeFromGate))
                .setLinearHeadingInterpolation(driveToGate1.getHeading(), intakeFromGate.getHeading())
                .build();

        intakePostToShoot3 = follower.pathBuilder()
                .addPath(new BezierLine(intakeFromGate, driveToShoot3))
                .setLinearHeadingInterpolation(intakeFromGate.getHeading(), driveToShoot3.getHeading())
                .build();

        gate2ToIntakePost = follower.pathBuilder()
                .addPath(new BezierLine(driveToGate2, intakeFromGate))
                .setLinearHeadingInterpolation(driveToGate2.getHeading(), intakeFromGate.getHeading())
                .build();

        intakePostToShoot4 = follower.pathBuilder()
                .addPath(new BezierLine(intakeFromGate, driveToShoot4))
                .setLinearHeadingInterpolation(intakeFromGate.getHeading(), driveToShoot4.getHeading())
                .build();
        // ============================================
    }
    ...
    public void autonomousPathUpdate() {
        switch (pathState) {
            ...
            case 4:
                ...
                if (!follower.isBusy()) {
                    if (runShootWindowQuick()) {
                        intakeSlow();
                        follower.followPath(goGate1Combined, true); // unchanged gate drive
                        setPathState(6);
                    }
                }
                break;
            ...
            case 6:
                if (!follower.isBusy()) {
                    // At gate: intake OFF, wait 0.5s, then move to intake post
                    intakeStop();
                    if (pauseTime(0.5)) {
            ...
            case 7:
                if (!follower.isBusy()) {
                    // At intake post: intake slow for existing pause time, then OFF and go shoot
                    intakeSlow();
                    if (pauseTime(1.5)) { // same pauseTime duration you already used here
                        intakeStop();
            ...
            case 8:
                // Shoot3 window (same as before)
                if (!follower.isBusy()) {
                    if (runShootWindowQuick()) {
                        intakeStop();
                        follower.followPath(goGate2Combined, true); // unchanged gate drive
                        setPathState(9);
                    }
                }
                break;
    ...
    public void loop() {
        ...
        if (!shooterSpinupStarted) {
            ...
            spinup.start(1170); //speed flywheel tickspersecond
    ...
    public static class SpinUpFastKickThenPIDF {
        ...
        private double targetTicksPerSec = 1100;
