...
import com.pedropathing.geometry.BezierCurve;   // <-- CHANGE (added)
...
@Autonomous(name = "Red Back V5 FastShot", group = "Examples")
public class RedBackV5FastCycleStayAtGate extends OpMode {
    ...
    public void buildPaths() {
        ...
        // ================= CHANGE: make gate1 a single smooth BezierCurve through driveTowardsGate1 =================
        ...
        // combined gate 2 (still the same as before)
    ...
    public void autonomousPathUpdate() {
        switch (pathState) {
            ...
            case 6:
                if (!follower.isBusy()) {
                    // CHANGE: run slow intake for 3 seconds BEFORE goShoot3
                    intakeSlow();
                    if (pauseTime(1.5)) {
            ...
            case 9:
                // Subsequent shoot: NO spinup, just 200ms settle + aim gate/timeout + pulse/feed
                if (!follower.isBusy()) {
                    // CHANGE: run slow intake for 3 seconds BEFORE goShoot4
                    intakeSlow();
                    if (pauseTime(1.5)) {
