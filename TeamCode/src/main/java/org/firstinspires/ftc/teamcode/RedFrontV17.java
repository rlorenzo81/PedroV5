...
@Autonomous(name = "RedFrontV17", group = "Examples")
public class RedFrontV17 extends OpMode {
    ...
    private static final double SHOOTER_TARGET_TPS = 1490; //shooter speed (CHANGE THIS) was 1445
    ...
    private static final double SHOOT_FEED_SEC = 2.1; // from RedBackV9 was 1.8
    ...
    private final Pose driveToShoot2 = new Pose(84, 22, Math.toRadians(65)); //y was 25 x was 83

    // (kept declared but NO LONGER USED in paths/state machine - same as BLUE)
    private final Pose toArtifactLine2 = new Pose(84, 22, Math.toRadians(0));
    private final Pose driveThroughLine2 = new Pose(130, 20, Math.toRadians(0));

    private final Pose driveToShoot3 = new Pose(84, 22, Math.toRadians(65)); //y was 25
    private final Pose cornerPickUp1 = new Pose(131, 20, Math.toRadians(315));     // TODO: tune
    private final Pose driveToShoot4 = new Pose(84, 22, Math.toRadians(65));      // TODO: tune (often same as shoot poses)
    private final Pose cornerPickUp2 = new Pose(130, 20, Math.toRadians(315));     // TODO: tune
    private final Pose driveToShoot5 = new Pose(84, 22, Math.toRadians(65));      // TODO: tune
    private final Pose leavePose = new Pose(100, 36, Math.toRadians(180));

    // ===== ADDED POSES FROM BLUE (placeholders you will tweak) =====
    // These do NOT exist in your original Red class, so I picked "reasonable" red-side values.
    private Path scorePreload;

    // Match BLUE’s “no line2 used” structure
    ...
    public void buildPaths() {
        ...
        // NEW: from goShoot2 we go straight to cornerPickUp1 (no line2 paths)  [FROM BLUE]
        ...
        // From cornerPickUp1 to shoot4 [FROM BLUE]
        ...
        // From shoot4 back to cornerPickUp2 [FROM BLUE]
        ...
        // From cornerPickUp2 to shoot5 [FROM BLUE]
        ...
        // Keep goShoot3 path definition, but now it starts from driveToShoot5 [FROM BLUE]
        ...
        // Finally leave pose (still from driveToShoot5) [FROM BLUE]
    ...
    public void autonomousPathUpdate() {
        switch (pathState) {
            ...
            case 1:
                ...
                if (!follower.isBusy()) {
                    if (runShootWindowFirst()) {
                        intakeSlow();
                        follower.followPath(driveToLine1, 0.8,true);
                        setPathState(2);
                    }
                }
                break;
            ...
            case 2:
                if (!follower.isBusy()) {
                    intakeSlow();
                    follower.followPath(pickUpLine1,0.8, true);
                    setPathState(3);
                }
                break;
            ...
            case 3:
                if (!follower.isBusy()) {
                    intakeHolds();
                    follower.followPath(goShoot2, 0.8, true);
                    setPathState(4);
                }
                break;
            ...
            case 4:
                ...
                if (!follower.isBusy()) {
                    if (runShootWindowQuick()) {
                        intakeSlow();
                        follower.followPath(goCorner1, 0.8,true);
                        setPathState(8);
                    }
                }
                break;
            ...
            case 8:
                ...
                if (!follower.isBusy()) {
                    intakeHolds();
                    follower.followPath(goShoot4, 0.8, true);
                    setPathState(9);
                }
                break;
            ...
            case 9:
                ...
                if (!follower.isBusy()) {
                    if (runShootWindowQuick()) {
                        intakeSlow();
                        follower.followPath(goCorner2, 0.8,true);
                        setPathState(10);
                    }
                }
                break;
            ...
            case 10:
                ...
                if (!follower.isBusy()) {
                    intakeHolds();
                    follower.followPath(goShoot5, 0.8, true);
                    setPathState(11);
                }
                break;
            ...
            case 11:
                ...
                if (!follower.isBusy()) {
                    if (runShootWindowQuick()) {
                        intakeStop();
                        follower.followPath(leaveOutChain,0.8, true);
                        setPathState(12);
                    }
                }
                break;
    ...
    public void loop() {
        ...
        telemetry.addData("savedTurretPos", turret.getSavedReturnPosition());
        telemetry.addData("hasSavedTurretPos", turret.hasSavedReturnPosition());
    ...
    public static class TurretTracker {
        ...
        private int savedReturnPosition = 0;
        private boolean hasSavedReturnPosition = false;

        private boolean visionMode = false;
        private double validSince = 0.0;
        private double invalidSince = 0.0;
        private double visionAcquireTime = 0.0;
        private int reacquireIgnoredFrames = 0;
        ...
        private static final double FF = 0.007;
        ...
        private static final double NO_MOVE_TX = 0.6;

        private static final double VALID_ACQUIRE_SEC = 0.22;
        private static final double INVALID_LOSS_SEC = 0.25;
        private static final double VISION_RAMP_SEC = 0.35;

        private static final int REACQUIRE_IGNORE_FRAMES = 4;
        private static final double REACQUIRE_MODE_SEC = 0.45;
        private static final double REACQUIRE_GAIN_SCALE = 0.40;
        private static final double REACQUIRE_NO_MOVE_TX = 1.4;

        // ===== OFFSET ADDED =====
        // Tune this to intentionally hold a little off the AprilTag center.
        // Start with small values like +/-1.0 to +/-3.0.
        private static final double TX_AIM_OFFSET = -6;

        // This is saved into the encoder-based return position so the saved lock can also carry an offset.
        // Leave at 0 if you only want to use TX_AIM_OFFSET.
        private static final int LOCK_OFFSET_TICKS = 0;

        // Return-to-saved-position hold tuning
        private static final double RETURN_KP = 0.009;
        private static final double RETURN_MAX_PWR = 0.55;
        private static final int RETURN_TOL_TICKS = 10;
        ...
        public void init(HardwareMap hw) {
            ...
            savedReturnPosition = 0;
            hasSavedReturnPosition = false;

            visionMode = false;
            validSince = 0.0;
            invalidSince = 0.0;
            visionAcquireTime = 0.0;
            reacquireIgnoredFrames = 0;
        }

        public void saveReturnPosition() {
            if (!hasSavedReturnPosition) {
                savedReturnPosition = turret.getCurrentPosition() + LOCK_OFFSET_TICKS;
                hasSavedReturnPosition = true;
            }
        }

        // compatibility with existing v15 calls
        ...
        public void update(double now, double dt, double moveBlend, double omegaDps) {
            ...
            boolean rawValid = (r != null && r.isValid());

            if (rawValid) {
                invalidSince = 0.0;
                if (validSince == 0.0) validSince = now;
            } else {
                validSince = 0.0;
                if (invalidSince == 0.0) invalidSince = now;
            }

            if (!visionMode) {
                if (rawValid && (now - validSince) >= VALID_ACQUIRE_SEC) {
                    // helper is done the instant vision is truly reacquired
                    visionMode = true;
                    visionAcquireTime = now;
                    reacquireIgnoredFrames = 0;

                    // fresh live-tracking controller state
                    aimHold = false;
                    aimStart = 0.0;
                    lastPower = 0.0;
                    ...
                    txFiltered = txPred;
                    lastErr = txPred - TX_AIM_OFFSET;

                    turret.setPower(0.0);
                    return;
                }
            } else {
                if (!rawValid && (now - invalidSince) >= INVALID_LOSS_SEC) {
                    visionMode = false;
                    visionAcquireTime = 0.0;
                    reacquireIgnoredFrames = 0;
                    aimHold = false;
            ...
            // saved encoder is only a temporary search position while vision is absent
            if (!visionMode) {
                aimHold = false;

                if (hasSavedReturnPosition) {
                    int currentPos = turret.getCurrentPosition();
                    int errorTicks = savedReturnPosition - currentPos;

                    double power;
                    if (Math.abs(errorTicks) <= RETURN_TOL_TICKS) {
                        power = 0.0;
                    } else {
                        power = clip(errorTicks * RETURN_KP, -RETURN_MAX_PWR, RETURN_MAX_PWR);
                    }
            ...
            if (reacquireIgnoredFrames < REACQUIRE_IGNORE_FRAMES) {
                reacquireIgnoredFrames++;
                txFiltered = txPred;
                lastErr = txPred - TX_AIM_OFFSET;
                turret.setPower(0);
                lastPower = 0;
            ...
            double error = txFiltered - TX_AIM_OFFSET;
            double errAbs = Math.abs(error);

            double gainScale = 1.0;
            double noMoveZone = NO_MOVE_TX;
            if ((now - visionAcquireTime) < REACQUIRE_MODE_SEC) {
                gainScale = REACQUIRE_GAIN_SCALE;
                noMoveZone = REACQUIRE_NO_MOVE_TX;
            }

            double dErr = (error - lastErr) / dt;
            lastErr = error;

            double p = (KP * gainScale) * error;
            double d = (KD * gainScale) * dErr;

            double ffScale = clip((errAbs - 1.5) / (8.0 - 1.5), 0.0, 1.0);
            double ff = (FF * gainScale) * omegaDps * (1.0 + moveBlend) * ffScale;
            ...
            if (errAbs < noMoveZone) {
                power = 0.0;
            }

            double visionRamp = clip((now - visionAcquireTime) / VISION_RAMP_SEC, 0.0, 1.0);
            power *= visionRamp;
        ...
        public int getSavedReturnPosition() { return savedReturnPosition; }
        public boolean hasSavedReturnPosition() { return hasSavedReturnPosition; }
    }
