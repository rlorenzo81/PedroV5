...
@Autonomous(name = "Red Front 11 Cycle", group = "Examples")
@Disabled
public class RedFrontV11Cycle extends OpMode {
    ...
    // KEEP Red shooter target value (you can change as needed)
    private static final double SHOOTER_TARGET_TPS = 1505;//was 1550

    // ================= FLYWHEEL SPEED GATE (FROM BLUE) =================
    ...
    // FROM BLUE
    private static final double AIM_TIMEOUT_SEC = .8;
    ...
    // FROM BLUE
    private static final double SHOOT_FEED_SEC = 1.8;

    // ================= SHOOTER SPIN-UP DELAY =================
    // FROM BLUE
    private static final double SHOOTER_SPINUP_SEC = 3.5;
    ...
    private final Pose driveToShoot2 = new Pose(84, 22, Math.toRadians(0)); //y was 25 x was 83
    ...
    private final Pose driveToShoot3 = new Pose(84, 22, Math.toRadians(0)); //y was 25
    private final Pose cornerPickUp1 = new Pose(130, 20, Math.toRadians(315));     // TODO: tune
    private final Pose driveToShoot4 = new Pose(84, 22, Math.toRadians(0));      // TODO: tune (often same as shoot poses)
    private final Pose cornerPickUp2 = new Pose(132, 20, Math.toRadians(315));     // TODO: tune
    private final Pose driveToShoot5 = new Pose(84, 22, Math.toRadians(0));      // TODO: tune
    ...
    private boolean runShootWindowFirst() {
        ...
        if (!shootingSequenceActive) {
            boolean speedOk = flywheelAtSpeed(now); // FROM BLUE (computed)
            if (aimOk || timedOut) {                // FROM BLUE (kept identical)
    ...
    // ================= AUTO STATE MACHINE (BLUE CYCLE FLOW) =================
    public void autonomousPathUpdate() {
        switch (pathState) {
            ...
            case 1:
                ...
                if (!follower.isBusy()) {
                    if (runShootWindowFirst()) {
                        intakeSlow();
                        follower.followPath(driveToLine1,0.6, true);
                        setPathState(2);
                    }
                }
                break;
            ...
            case 2:
                if (!follower.isBusy()) {
                    intakeSlow();
                    follower.followPath(pickUpLine1, 0.6,true);
                    setPathState(3);
                }
                break;
            ...
            case 4:
                // After goShoot2 we go to cornerPickUp1 (removed line2) [FROM BLUE]
                if (!follower.isBusy()) {
                    if (runShootWindowQuick()) {
                        intakeSlow();
                        follower.followPath(goCorner1,0.6, true);
                        setPathState(8);
                    }
                }
                break;
            ...
            case 8:
                // Arrived cornerPickUp1, now go to shoot 4 [FROM BLUE]
                if (!follower.isBusy()) {
                    intakeHolds();
            ...
            case 9:
                // Shoot 4, then go back to cornerPickUp2 with intakeSlow [FROM BLUE]
                if (!follower.isBusy()) {
                    if (runShootWindowQuick()) {
                        intakeSlow();
                        follower.followPath(goCorner2, 0.6,true);
                        setPathState(10);
                    }
                }
                break;
            ...
            case 10:
                // Arrived cornerPickUp2, now go to shoot 5 [FROM BLUE]
                if (!follower.isBusy()) {
                    intakeHolds();
    ...
    // ================= SHOOTER: RAMP-VELOCITY (FROM BLUE) =================
    private SpinUpRampVelocity spinup;
    private boolean shooterSpinupStarted = false;

    // ================= TELEOP-STYLE FLYWHEEL CONTROL (ADDED EXACTLY AS TELEOP) =================
    ...
    public void loop() {
        ...
        // =========================
        // TELEOP-STYLE FLYWHEEL CONTROL (RAMP VELOCITY ONLY, SETPOINT-BASED)
        // =========================
        boolean shooterOn = (shooterSetpoint > 0.0);

        if (shooterOn) {

            // Range-dependent ramp timing (same pattern as TeleOp)
    ...
    public void init() {
        ...
        // NEW: ramp-velocity spinup (matches blue logic)
        ...
        // TeleOp-style flywheel init state
        shooterSetpoint = 0.0;
        shooterTargetVel = 0.0;
        shooterTargetVelCmd = 0.0;
        shooterWasOnDiag = false;
    ...
    public void start() {
        ...
        // TeleOp-style: set the flywheel setpoint at match start
        shooterSetpoint = SHOOTER_TARGET_TPS;
        shotRangeMode = ShotRangeMode.CLOSE;
    ...
    // ================= TURRET TRACKER (KEEP YOUR RED VERSION) =================
    public static class TurretTracker {
        ...
        private static final double KP = 0.055;//was 0.055
