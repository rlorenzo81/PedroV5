...
@Autonomous(name = "Red Front V4", group = "Examples")
public class RedFrontV4 extends OpMode {
        ...
        switch (pathState) {
            ...
            case 1:
                if (!follower.isBusy()) {
                    ...
                    // Feed only when turret is aimed and stable
                    if (turret.isAimedStable(getRuntime())) {

                        intakeShootFeed();

                        // balls time to be shot before the robot moves again
                        if (pauseTime(1.5)) {
                            intakeStop();
                            pathState = -1; // stop after preload in this simplified V4
                        }
                    }
                }
                break;
    ...
    // ================= TURRET TRACKER (WITH RECOMMENDED SIGN FIXES) =================
    public static class TurretTracker {
        ...
        // --- Proper derivative state ---
        private double lastErr = 0;

        // =========================================================
        // RECOMMENDED SIGN KNOBS
        // =========================================================
        // If turret turns the WRONG way when tx > 0, flip TX_SIGN to -1.
        ...
        // Feedforward base (will be scaled down near center)
        ...
        // Aiming gate
        private static final double AIM_TOL = 1.5;
        private static final double AIM_PWR_TOL = 0.08;
        private static final double AIM_STABLE = 0.18;
        ...
        public void update(double now, double dt, double moveBlend, double omegaDps) {
            ...
            // txPred is already sign-corrected here via TX_SIGN, and lead direction is controlled by LEAD_SIGN
            double txPred = (TX_SIGN * txRaw) + (LEAD_SIGN * omegaDps * LEAD_TIME);

            // 2) Smoothing (more smoothing to prevent overshoot)
            double alpha = (moveBlend > 0.2) ? 0.18 : 0.55;
